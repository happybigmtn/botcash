"""Database models for Botcash Discord Bridge."""
...
class LinkedIdentity(Base):
    """Links a Discord user to a Botcash address."""
    __tablename__ = "linked_identities"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    # Discord user IDs are snowflakes (64-bit integers)
    ...
    discord_discriminator: Mapped[Optional[str]] = mapped_column(String(4), nullable=True)
...
class RelayedMessage(Base):
    ...
    # Direction: "discord_to_bc" or "bc_to_discord"
    direction: Mapped[str] = mapped_column(String(16), nullable=False)

    # Source identifiers (Discord snowflakes are 64-bit)
    ...
    discord_guild_id: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint("discord_message_id", "discord_channel_id", name="uq_discord_message"),
...
class RateLimitEntry(Base):
    ...
    __table_args__ = (
        UniqueConstraint("discord_user_id", "window_start", name="uq_user_window"),
    )
...
class BridgedChannel(Base):
    """Configuration for bridged Discord channels."""
    __tablename__ = "bridged_channels"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    discord_guild_id: Mapped[int] = mapped_column(BigInteger, nullable=False, index=True)
    discord_channel_id: Mapped[int] = mapped_column(BigInteger, unique=True, nullable=False, index=True)
    channel_name: Mapped[str] = mapped_column(String(100), nullable=False)

    # Bridge settings
    relay_posts: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    relay_replies: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    auto_post_format: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
