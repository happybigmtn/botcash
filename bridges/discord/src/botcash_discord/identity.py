"""Identity linking service for Discord <-> Botcash bridge."""
...
class IdentityService:
    """Service for managing identity links between Discord and Botcash."""
    ...
    async def get_linked_identity(
        ...
        """Get linked identity for Discord user.
        ...
        result = await session.execute(
            select(LinkedIdentity).where(
                LinkedIdentity.discord_user_id == discord_user_id,
    ...
    async def initiate_link(
        ...
        Args:
            ...
            discord_discriminator: Discord discriminator (optional, deprecated in new system)
        ...
        # Check if address is already linked to another Discord user
        ...
        if existing_by_address.scalar_one_or_none():
            raise IdentityLinkError("This Botcash address is already linked to another Discord account")

        # Check if Discord user already has an active link
        ...
        if existing:
            ...
            if existing.status == LinkStatus.ACTIVE:
                raise IdentityLinkError(
                    "You already have a linked address. Use /bcash_unlink first."
                )
        ...
        verification_msg = (
            f"I am linking Discord user {discord_user_id} to Botcash.\n"
    ...
    async def complete_link(
        ...
        if not identity:
            raise IdentityLinkError("No pending link found. Use /bcash_link first.")
        ...
        if identity.challenge_expires_at:
            ...
            if datetime.now(timezone.utc) > expires_at:
                raise IdentityLinkError("Challenge expired. Please start over with /bcash_link")
        ...
        link_result = await self.botcash.create_bridge_link(
            ...
            platform="discord",
            platform_id=str(discord_user_id),
            proof=signature,
        )
    ...
    async def unlink(
        ...
        """Unlink Discord account from Botcash address.
